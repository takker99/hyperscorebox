...
export class MIDIPlayer {
  ...
  private smf: ArrayBuffer = new Uint8Array(0);
  ...
  constructor(smf: ArrayBuffer) {
    ...
    this.playAndPauseButton = this.createButton(
      "midi-play-button",
      this.playPause,
    );
  ...
  private seek = (): void => {
    ...
    const positionSeekTo = Math.round(
      Number(this.seekBar.value) / 100 * maxTick,
    );
  ...
  private download = (): void => {
    const url = URL.createObjectURL(
      new Blob([this.smf], { type: "audio/midi" }),
    );
    window.open(url);
  };
