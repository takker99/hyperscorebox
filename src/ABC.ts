...
const generateClickListener = (links: ABCLink[], parentSVGElID: string) => {
  return (abcElem, tuneNumber, classes): void => {
    ...
    if (pageTitle) {
      console.log(
        "abcClickListener",
        "Linked note is clicked.",
        "startChar:",
        clickedNoteStartChar,
        "destination:",
        pageTitle,
      );
    ...
    document.execCommand(
      "insertText",
      null,
      addLinkToABC(clickedNoteStartChar),
    );
    // onInput();
  };
};
...
export const getSMF = (containerEl: Element): ArrayBuffer => {
  ...
  const midiStr = midiDlEl.getAttribute("href").replace(/data:audio\/midi,/, "")
    .replace(/MThd/g, "%4D%54%68%64").replace(/MTrk/g, "%4D%54%72%6B");
