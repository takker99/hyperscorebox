import { ABCBlock, ScoreView } from "./Types.ts";
import { generateInlineStyle } from "./Scrapbox.ts";
import { getSMF, parseLink, render } from "./ABC.ts";
...
export class ScoresInPage {
  ...
  private updateElement = (block: ABCBlock): boolean => {
    ...
    scoreView.element.setAttribute(
      "style",
      generateInlineStyle(isEditing, blockHeight, offsetLeft, width),
    );
    render(
  ...
  public update = (
    newAbcBlocks: ABCBlock[],
    isPageTransition: boolean = false,
  ): void => {
